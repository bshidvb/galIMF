...
def plot_noeg_gal_sfr():
    ...
    points = [
        ...
        (3e6, 2, 'Mrk 996 (low density)', cmap(1), 0.5e6, 0.0, 0.00544, False), #sfr from Ha luminosity, James+09
        (3.5e6, 3.3, 'LyC', cmap(2), 0.9e6, 0.11, 2.37, False), #Vanzella+21
        (100e6, 10, 'EXCELS-121806 - uncertain age', cmap(10), 0.0, 1, 5.225, False), #Arellano-Cordova+25
        ...
        (26e6, 40, 'GS_9422 (tentative)', cmap(13), 0.0, 0.0, 5.943, True), #(t=t50*2), Tacchella+24 -> Simmonds+24 from SED
        (26e6, 5.4, 'GS_9422 (tentative)', cmap(16), 0.0, 0.1, 5.943, True), #(t=t50*2), Tacchella+24 -> Scholtz+23
        (13e6, 2.5, 'RXJ2248-ID', cmap(15), 12e6, 2.0, 6.1057, False), #Topping+24
        (10e6, 11.3, 'GN-z8-LAE', cmap(18), 0.0, 7.8, 8.279, False), #sfr from Ha, Navarro-Carrera+24
        (68e6, 30, 'CEERS_1019', cmap(18), 29e6, 1.5, 8.679, True), #mass-weighted age (t=t50*2), Larson+23, sfr from Hb
        (10e6, 12, 'GN-z11', cmap(3), 0, 10, 10.6, True), #Tacchella+23
        (30e6, 21, 'GN-z11', cmap(3), 0, 26, 10.6, True), #Tacchella+23
        (12e6, 4.34, 'GS-z9-0 light-weighted', cmap(11), 2e6, 0.1, 9.4327, True), #Curti+25
        (32e6, 5.5, 'GS-z9-0 mass-weighted', cmap(11), 20e6, 0.2, 9.4327, True), #Curti+25
        (28e6, 5.2, 'GHZ2/GLASS-z12', cmap(4), 10e6, 1.1, 12.34, True), #mass-weighted age, Castellano+24
        #(75e6, 19, 'JADES-GS-z14-0', cmap(6), 25e6, 0.0, 14.32, ?), #SED fitting, Carniani+25
    ]
        
    handles = []
    for age, sfr, label, color, age_err, sfr_err, z, agn_signatures in points:
        age = age/1e9
        age_err = age_err/1e9
        y = np.log10(sfr)
        # compute asymmetric log errors from linear age errors (guard against age_err==0)
        ...
        print(age,y)
        if agn_signatures == True:
            sc = plt.scatter(age, y, color=color, label=label, s=20, marker="x", zorder=4)
            plt.errorbar(age, y, xerr=age_err, yerr=yerr_plot,
                     fmt='none', elinewidth=0.8, ecolor=color, zorder=3)
            handles.append(sc)
        else:
            sc = plt.scatter(age, y, color=color, label=label, s=20, marker="p", zorder=4)
            plt.errorbar(age, y, xerr=age_err, yerr=yerr_plot,
                     fmt='none', elinewidth=0.8, ecolor=color, zorder=3)
            handles.append(sc)

    lgd = plt.legend(handles=handles, labels=[p[2] for p in points],
                     bbox_to_anchor=(1.01, 0.85), loc='upper left', fontsize=8,
                     title="Observational Data")
    return lgd
...
def plot_NO_over_time(file_paths, labels):
    ...
    plt.savefig('/Users/adriana_work/Desktop/galIMF/figs/galevo_output_plots/N_over_O_over_time.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def plot_sfh_with_data(file_paths,labels, min, max):
    ...
    plt.gca().add_artist(lgd2)
    plot_noeg_gal_sfr()
    plt.savefig('/Users/adriana_work/Desktop/galIMF/figs/galevo_output_plots/sfh.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def plot_sfh(file_paths,labels, min, max):
    ...
    plt.title('Star Formation History', fontsize=16)
    plt.legend()
    plt.savefig('/Users/adriana_work/Desktop/galIMF/figs/galevo_output_plots/sfh_without_data.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def metallicity_abundance_evolution(file_paths):
    data = load_data_with_names(file_paths)
    time_axis = data['time step list:']
    gas_NO = data['Gas metallicity, [Z/X]:']
    stellar_NO = data['Stellar mass-weighted metallicity, [Z/X]:']
    stellar_NO_luminosity_weighted = data['Stellar luminosity-weighted metallicity, [Z/X]:']
    plt.rc('font', family='serif')
    plt.rc('xtick', labelsize='x-small')
    plt.rc('ytick', labelsize='x-small')
    plt.plot(time_axis, gas_NO, label='gas')
    ...
    plt.xlabel(r'log$_{10}$(time [yr])')
    plt.ylabel('N/O')
    plt.title('Metallicity history', fontsize=10)
    plt.legend(prop={'size': 7})
    plt.tight_layout()
    plt.show()
