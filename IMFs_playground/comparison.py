...
kroupa_imf_unnormalized_values = [kroupa_imf_unnormalized(m) for m in masses]
...
custom_imf_unnormalized_values = [custom_imf_unnormalized(m) for m in masses]
...
plt.loglog(masses, kroupa_imf_unnormalized_values, label="Kroupa IMF Unnormalized", color="red")
plt.loglog(masses, given_imf_values, label="Given IMF", color="purple")
plt.loglog(masses, custom_imf_values, label="Custom Salpeter IMF", color="orange")
plt.loglog(masses, custom_imf_unnormalized_values, label="Custom Salpeter IMF Unnormalized", color="brown")
...
def cumulative_number_of_stars(imf_function, masses):
    cumulative = []
    for m in masses:
        result, _ = quad(imf_function, masses[0], m)
...
def cumulative_mass_of_stars(imf_function, masses):
    cumulative = []
    for m in masses:
        result, _ = quad(lambda x: x * imf_function(x), masses[0], m)
        cumulative.append(result)
    return cumulative
...
kroupa_unnormalized_cumulative_number = cumulative_number_of_stars(kroupa_imf_unnormalized, masses)
kroupa_unnormalized_cumulative_mass = cumulative_mass_of_stars(kroupa_imf_unnormalized, masses)
...
custom_unnormalized_cumulative_number = cumulative_number_of_stars(custom_imf_unnormalized, masses)
custom_unnormalized_cumulative_mass = cumulative_mass_of_stars(custom_imf_unnormalized, masses)
...
plt.loglog(masses, kroupa_unnormalized_cumulative_number, label="Kroupa IMF Unnormalized", color="red")
...
plt.loglog(masses, custom_unnormalized_cumulative_number, label="Custom Salpeter IMF Unnormalized", color="brown")
plt.xlabel("Stellar Mass (M☉)")
...
plt.loglog(masses, kroupa_unnormalized_cumulative_mass, label="Kroupa IMF Unnormalized", color="red")
...
plt.loglog(masses, custom_unnormalized_cumulative_mass, label="Custom Salpeter IMF Unnormalized", color="brown")
plt.xlabel("Stellar Mass (M☉)")
